...
class GeminiService:
    ...
    def explain_term(
        self, term: str, category: str = None, model_name: str = None
    ) -> str:
        ...
        try:
            model = (
                ...
                if model_name
                else self.get_model('gemini-1.5-pro')
            )
            ...
            raise f'Failed to explain term: {str(e)}'
    ...
    def translate_text(self, prompt: str, model_name: str = None) -> str:
        ...
        try:
            model = (
                self.get_model(model_name)
                if model_name
                else self.get_model('gemini-1.5-flash')
            )
